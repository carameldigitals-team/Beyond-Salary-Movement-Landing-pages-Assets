...
export const HeaderNav: React.FC<HeaderNavProps> = ({ onStartScorecard, showCta = true }) => {
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#E1E7F7]">
      <div className="max-w-[1080px] mx-auto px-5 py-3 md:py-3.5 flex items-center justify-between">
        ...
        {showCta && (
          <button 
            onClick={onStartScorecard}
            className="group flex items-center gap-2 bg-[#03037E] hover:bg-[#020254] text-white font-bold text-xs md:text-sm px-4 md:px-5 py-2.5 rounded-full transition-all duration-200 active:scale-95 shadow-sm"
          >
